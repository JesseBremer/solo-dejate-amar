...
@Component({
  ...
  template: `
    <div class="flex flex-col items-center w-full px-4 pt-8 pb-6">
      <h1 class="text-romantic-coral font-romantic text-4xl md:text-5xl text-center mb-1">{{ t().vault_title }}</h1>
      <p class="text-romantic-text/60 text-sm font-serif italic mb-8 text-center">{{ t().vault_subtitle }}</p>

      @if (vaultService.messages().length === 0) {
        <div class="flex flex-col items-center gap-3 mt-16 text-center">
          <span class="text-5xl">🔐</span>
          <p class="text-romantic-text/60 font-serif italic text-sm" [innerHTML]="t().vault_empty.replace('\\n','<br>')"></p>
        </div>
      }
      ...
      @if (sealed().length > 0) {
        <div class="w-full max-w-[600px] mb-8">
          <div class="flex items-baseline gap-2 mb-3">
            <span class="text-romantic-text/50 text-xs font-serif uppercase tracking-widest">{{ t().vault_sealed }}</span>
      ...
      @if (opened().length > 0) {
        <div class="w-full max-w-[600px]">
          <div class="flex items-baseline gap-2 mb-3">
            <span class="text-romantic-text/50 text-xs font-serif uppercase tracking-widest">{{ t().vault_open_letters }}</span>
                    ...
                    <div class="flex-1 min-w-0">
                      ...
                      <div class="flex items-center gap-1.5 mt-0.5">
                        ...
                        <span class="text-romantic-text/50 text-[11px] font-serif">
                          {{ msg.author === 'jesse' ? 'Jesse' : 'Abigail' }} · {{ t().vault_written }} {{ formatDate(msg.created_at) }}
                        </span>
                      </div>
                    </div>
                    <button (click)="toggleMenu(msg.id)"
